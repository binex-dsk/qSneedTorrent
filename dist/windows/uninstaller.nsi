...
; Remove files and uninstaller
  Delete "$INSTDIR\qsneedtorrent.exe"
  Delete "$INSTDIR\qsneedtorrent.pdb"
...
; Remove shortcuts, if any
  RMDir /r "$SMPROGRAMS\qSneedTorrent"
  Delete "$DESKTOP\qSneedTorrent.lnk"
SectionEnd

Section "un.$(remove_associations)" ;"un.Remove file associations"
  SectionIn RO
  ReadRegStr $0 HKLM "Software\Classes\.torrent" ""
  StrCmp $0 "qSneedTorrent" 0 torrent_end
  ...
  StrCmp $0 '"$INSTDIR\qsneedtorrent.exe" "%1"' 0 magnet_end
...
Section "un.$(remove_registry)" ;"un.Remove registry keys"
  ...
  DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\qSneedTorrent"
  DeleteRegKey HKLM "Software\qSneedTorrent"
  DeleteRegKey HKLM "Software\Classes\qSneedTorrent"
...
Section "un.$(remove_firewall)" ;
  ...
  nsisFirewallW::RemoveAuthorizedApplication "$INSTDIR\qsneedtorrent.exe"
